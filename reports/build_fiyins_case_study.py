...
def write_markdown(data: dict, chart1: Path, chart2: Path) -> Path:
    ...
    if rule_wins:
        ...
        md.append("- The rule-based 5% stop did not beat buy-and-hold on any ticker in the portfolio (consistent with the formal eight-ticker basket result in §5.5 of the dissertation).")
    md.append("")
    md.append("---")
    md.append("")
    ...
    md.append("formal objective (§3.1.5) is *maximise risk-adjusted return subject to a ")
    ...
    md.append("their respective ways (§2.1 of the dissertation).")
    md.append("")
    ...
    md.append("of activity itself eats a few basis points). The dissertation's §6.3 names these ")
    ...
    md.append("§6.3 names these regimes explicitly and points at sector-aware uncertainty ")
    ...
    md.append("- **Equal-weight aggregation.** The portfolio-level row in §2 is an unweighted mean across tickers, not a true rebalanced equal-weight portfolio. Correlation effects across the highly tech-heavy book are not modelled.")
